...
replicate(n = 1000, norTest(testList, n = 10) <= 0.05) %>% apply(MARGIN = c(1,2), sum)
...
table(pi1M) %>% 
  barplot(main = "Frequence of Pi to 1M Decimal Places", xlab = "Number", ylab = "Frequency")
...
gap.test((pi1M+1)/10, 0.25, 0.75)
testGap <- gap.test(pi1M/9, echo = TRUE)
print(testGap)

c(table(pi1M)[1:5], sum(table(pi1M)[5:10]))
